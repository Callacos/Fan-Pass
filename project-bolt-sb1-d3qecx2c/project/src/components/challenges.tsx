...
interface ChallengeWithPosition extends Challenge {
...
const Challenges: React.FC<ChallengesProps> = ({ onBackToMenu, onOpenCollection }) => {
  ...
  const [flippedChallenges, setFlippedChallenges] = useState<Set<number>>(new Set());

  const calculateChallengePosition = (index: number): ChallengePosition => {
  ...
  const filterChallenges = useCallback((filter: string) => {
  ...
  const visibleChallenges = currentChallenges.filter((challenge: Challenge) => !challenge.hidden);
  const challengesWithPositions: ChallengeWithPosition[] = visibleChallenges.map((challenge: Challenge, index: number) => ({
    ...challenge,
        ...
        >
          {challengesWithPositions.map((challenge, index) => (
            <div
              ...
              onClick={() => handleChallengeClick(challenge.id || 0)}
            >
              <div className={`hex-container ${flippedChallenges.has(challenge.id || 0) ? 'flipped' : ''}`}>
